...
void MuestraUnaPartida(stPersonaje *aux); /**Muestra las partidas, una parte de stPersonaje*/
...
int CompraEscudo(stPersonaje *player, int escudocant)
{
    ...
    if(control == 's')
    {
        ...
        if( player->inv.dinero >= compra)
        {
            ...
            player->inv.dinero = player->inv.dinero - compra;
            escudocant = 0;
            strcpy(player->inv.escudo,"Escudo de Luz");
            printf("Gracias por su compra!\n");
        }
        else
        {
...
int CompraHacha(stPersonaje *player, int hachacant)
{
    ...
    if(control == 's')
    {
        ...
        if( player->inv.dinero >= compra)
        {
            ...
            player->inv.dinero = player->inv.dinero - compra;
            hachacant = 0;
            strcpy(player->inv.arma,"Hacha de guerra");
            printf("Gracias por su compra!\n");
        }
        else
        {
...
void GuardarProgreso(stPersonaje *player)
{
    ...
    /**consultar, cada registro es un "slot", si guarda por primera vez va al primero,
    si ya ha guardado antes debe abrir en modo append**/
    ...
    while(flagGame == 1)
    {
        ...
        case 1:
            do
            {
                ...
                pasaNivel = CicloPelea(player,60,10,"FireSpawn");
...
void MuestraArchivoSave()
{
    ...
    if(pfile!=NULL)
    {
        if((fread(&aux,DIM,1,pfile)) > 0 )
        {
            rewind(pfile);

            printf("LAS PARTIDAS SON:\n");
...
void MuestraUnaPartida(stPersonaje *aux)
{
    printf("*********************************************************\n");
    printf("Nombre.................: %s\n",aux->nombre);
    printf("Clase..................: %s\n",aux->tipoClase);
